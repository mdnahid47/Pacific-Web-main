...
app.get('/api/admin/vendors', verifyToken(['admin', 'superadmin']), async (req, res) => {
  try {
    ...
    // JSON ফিল্ড পার্স করুন এবং URL নরমালাইজ করুন
    const parsedVendors = vendors.map(vendor => {
      const parsed = { ...vendor };
      
      // ডকুমেন্ট URL গুলো নরমালাইজ করুন
      if (parsed.nid_front) {
        parsed.nid_front = getFullFileUrl(parsed.nid_front);
      }
      if (parsed.nid_back) {
        parsed.nid_back = getFullFileUrl(parsed.nid_back);
      }
      if (parsed.cv) {
        parsed.cv = getFullFileUrl(parsed.cv);
      }
      if (parsed.trade_license) {
        parsed.trade_license = getFullFileUrl(parsed.trade_license);
      }
      if (parsed.photo) {
        parsed.photo = getFullFileUrl(parsed.photo);
      }
