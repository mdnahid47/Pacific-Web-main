...
//                       <td>
//                         <span className={`px-2 py-1 rounded-full text-xs font-medium ${user.isActive ? "bg-green-500/20 text-green-500" : "bg-red-500/20 text-red-500"
//                           }`}>
...
//                     </div>
//                   </div>
//                   <span className={`px-3 py-1 rounded-full text-xs sm:text-sm font-medium ${viewUser.isActive ? "bg-green-500/20 text-green-500" : "bg-red-500/20 text-red-500"
//                     }`}>
...
//     onClick={onClick}
//     className={({ isActive }) =>
//       `flex items-center p-3 rounded-lg ${isActive || active ? "bg-primary text-white" : "hover:bg-gray-700"
//       }`
//     }
//   >
  ...
  return (
    <div className="flex flex-col lg:flex-row min-h-screen bg-gray-900 text-white relative">
      ...
      <main className="flex-1 p-4 lg:p-6 lg:ml-0">
        ...
        <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center mb-6 gap-4">
          <div className="relative w-full lg:w-72">
            ...
            <input
              ...
              value={search}
              onChange={(e) => setSearch(e.target.value)}
              className="input input-bordered w-full pl-10 bg-gray-800 border-gray-700 focus:border-primary text-white"
            />
          </div>
            ...
            <table className="table w-full">
              <thead>
                <tr className="border-b border-gray-700 bg-gray-900/50">
                  <th className="text-gray-400 font-medium">User ID</th>
                  <th className="text-gray-400 font-medium">User</th>
                  <th className="text-gray-400 font-medium hidden sm:table-cell">Email</th>
                  <th className="text-gray-400 font-medium hidden md:table-cell">Phone</th>
                  <th className="text-gray-400 font-medium">Joined</th>
                ...
                {currentUsers.length > 0 ? (
                  currentUsers.map((user) => (
                    <tr key={user._id} className="border-b border-gray-700 hover:bg-gray-700/50 transition-colors">
                      <td className="font-mono text-sm">#{user._id.slice(-6)}</td>
                      <td>
                      ...
                      <td className="hidden sm:table-cell text-gray-300">{user.email}</td>
                      <td className="hidden md:table-cell text-gray-300">{user.phone || "N/A"}</td>
                  ...
                  <tr>
                    <td colSpan="7" className="text-center py-8 text-gray-400">
                      <div className="flex flex-col items-center justify-center">
                        <FiUsers className="text-4xl text-gray-500 mb-4" />
                        <h3 className="text-lg font-medium text-gray-300 mb-2">No users found</h3>
                        <p className="text-gray-400">
                          {search ? 'Try changing your search criteria' : 'No users have registered yet'}
                        </p>
                      </div>
                    </td>
                  </tr>
                ...
                {[...Array(Math.min(5, totalPages)).keys()].map((_, idx) => {
                  ...
                  return (
                    <button
                      ...
                      className={`btn btn-xs sm:btn-sm ${currentPage === pageNum ? 'btn-primary' : 'btn-ghost'}`}
                    >
                      {pageNum}
                    </button>
                  );
        ...
        {/* View User Modal */}
        <dialog id="view_user_modal" className="modal modal-bottom sm:modal-middle">
          <div className="modal-box max-w-4xl bg-gray-800 border border-gray-700 max-h-[90vh] overflow-y-auto">
            <form method="dialog">
              <button className="btn btn-sm btn-circle btn-ghost absolute right-4 top-4 text-white hover:bg-gray-700">✕</button>
            </form>
            ...
            {viewUser && (
              <div className="space-y-6">
                    ...
                    <div className="avatar">
                      <div className="w-16 h-16 rounded-full bg-gray-700 flex items-center justify-center">
                        {viewUser.profileImage ? (
                          <img src={viewUser.profileImage} alt={viewUser.name} className="w-full h-full object-cover rounded-full" />
                        ) : (
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6">
                  {/* User Information */}
                  <div className="card bg-gray-700/50 p-3 sm:p-4">
                    <h4 className="font-semibold text-lg mb-2 sm:mb-3 text-white">User Information</h4>
                    <div className="space-y-1 sm:space-y-2 text-sm sm:text-base">
                      <p><span className="text-gray-400">User ID:</span> <span className="text-white">{viewUser._id}</span></p>
                      <p><span className="text-gray-400">Email:</span> <span className="text-white">{viewUser.email}</span></p>
                      <p><span className="text-gray-400">Phone:</span> <span className="text-white">{viewUser.phone || "N/A"}</span></p>
                      <p><span className="text-gray-400">Joined:</span> <span className="text-white">{new Date(viewUser.createdAt).toLocaleDateString()}</span></p>
                      {viewUser.role && (
                        <p><span className="text-gray-400">Role:</span> <span className="text-white">{viewUser.role}</span></p>
                      )}
                    </div>
                  </div>
                  ...
                  {/* Order Stats */}
                  <div className="card bg-gray-700/50 p-3 sm:p-4">
                    <h4 className="font-semibold text-lg mb-2 sm:mb-3 text-white">Order Statistics</h4>
                    <div className="grid grid-cols-2 gap-2 sm:gap-3 text-sm sm:text-base">
                      <div className="bg-gray-600/30 p-2 rounded">
                        <p className="text-gray-400">Total Orders</p>
                        <p className="font-bold text-white">{userOrders.length}</p>
                      </div>
                      <div className="bg-gray-600/30 p-2 rounded">
                        <p className="text-gray-400">Completed</p>
                  ...
                  {viewUser.home_address && (
                    <div className="card bg-gray-700/50 p-3 sm:p-4">
                      <h4 className="font-semibold text-lg mb-2 sm:mb-3 text-white">Home Address</h4>
                      <AddressDisplay address={viewUser.home_address} />
                    </div>
                  )}
                  ...
                  {viewUser.office_address && (
                    <div className="card bg-gray-700/50 p-3 sm:p-4">
                      <h4 className="font-semibold text-lg mb-2 sm:mb-3 text-white">Office Address</h4>
                      <AddressDisplay address={viewUser.office_address} />
                    </div>
                  )}
                </div>
                ...
                {/* Recent Orders */}
                <div className="card bg-gray-700/50 p-3 sm:p-4">
                  <h4 className="font-semibold text-lg mb-2 sm:mb-3 text-white">Recent Orders</h4>
                  <div className="overflow-x-auto">
                    <table className="table table-compact w-full">
                      <thead>
                        ...
                        {userOrders.length > 0 ? (
                          userOrders.slice(0, 5).map(order => (
                            <tr key={order.order_id} className="border-b border-gray-700">
                              <td className="text-white font-mono text-sm">{order.order_id}</td>
                              <td className="text-gray-300">{new Date(order.order_date).toLocaleDateString()}</td>
                              <td>
                                <span
                                  className={`badge ${order.status === "Completed"
                                      ? "badge-success"
                              ...
                              <td className="text-white font-bold">৳{order.total}</td>
                            </tr>
                          ))
                        ) : (
                          <tr>
                            <td colSpan="4" className="text-center text-gray-400 py-4">No orders found</td>
                          </tr>
                        )}
                      </tbody>
                    </table>
        ...
        <dialog id="edit_user_modal" className="modal modal-bottom sm:modal-middle">
          <div className="modal-box max-w-2xl bg-gray-800 border border-gray-700">
            ...
            {editUser && (
              <div>
                <h3 className="text-xl font-bold mb-4 sm:mb-6 text-white">Edit User #{editUser._id.slice(-6)}</h3>
...
const SidebarLink = ({ to, icon, text, onClick, active = false }) => (
  <NavLink
    ...
    onClick={onClick}
    className={({ isActive }) =>
      `flex items-center p-3 rounded-lg transition-colors ${isActive || active ? "bg-primary text-white" : "hover:bg-gray-700 text-gray-300"
      }`
    }
  >
...
// Reusable StatsCard component
const StatsCard = ({ title, value, icon, color = "" }) => (
  <div className="card bg-gray-800 shadow-sm border border-gray-700">
    <div className="card-body p-2 sm:p-3">
      <div className="flex items-center justify-between">
        <h3 className="text-gray-400 text-xs sm:text-sm">{title}</h3>
        <span className={`text-lg ${color}`}>{icon}</span>
...
const AddressDisplay = ({ address }) => {
  ...
  return (
    <div className="space-y-1 sm:space-y-2 text-sm sm:text-base">
      {addressObj.full_address && <p className="text-white">{addressObj.full_address}</p>}
      <div className="grid grid-cols-2 gap-1">
        {addressObj.houseNo && <p><span className="text-gray-400">House:</span> <span className="text-white">{addressObj.houseNo}</span></p>}
        {addressObj.roadNo && <p><span className="text-gray-400">Road:</span> <span className="text-white">{addressObj.roadNo}</span></p>}
        {addressObj.area && <p><span className="text-gray-400">Area:</span> <span className="text-white">{addressObj.area}</span></p>}
        {addressObj.thana && <p><span className="text-gray-400">Thana:</span> <span className="text-white">{addressObj.thana}</span></p>}
        {addressObj.district && <p><span className="text-gray-400">District:</span> <span className="text-white">{addressObj.district}</span></p>}
        {addressObj.division && <p><span className="text-gray-400">Division:</span> <span className="text-white">{addressObj.division}</span></p>}
      </div>
    </div>
  );
};
