...
const LeftSide = () => {
    ...
    const fetchUserData = async () => {
      try {
        ...
        if (response.status === 200) {
          ...
          // Populate address fields if user has saved address
          if (response.data.address) {
            setHouseNo(response.data.address.houseNo || '');
            setRoadNo(response.data.address.roadNo || '');
            setBlockNo(response.data.address.blockNo || '');
            setSectorNo(response.data.address.sectorNo || '');
            setAreaName(response.data.address.areaName || '');
  ...
  useEffect(() => {
    const isValid = houseNo.trim() && roadNo.trim() && areaName.trim() && phoneNumber.trim();
    ...
    <div>
      ...
      {userDetails && (
        <div className="mt-4 p-4 bg-gray-100 rounded-lg">
          ...
          <p>Phone: {phoneNumber || userDetails.phone}</p>
        </div>
      )}
